...
class Parser():
    def __init__(self,file):
        ...
        if self.data['tipo'] == 'BLACK':
            self.cuenta = cuenta.Cuenta(100000,10000000000000,0,0,-10000)
            self.cliente= cliente.ClienteBlack(self.data)
            #print('Se creo BLACK')
            #print(self.cuenta)
        elif self.data['tipo'] == 'GOLD':
            self.cuenta = cuenta.Cuenta(20000,500000,0,0.5,-10000)
            self.cliente= cliente.ClienteGold(self.data)
            #print('Se creo GOLD')
            #print(self.cuenta)
        else:
            self.cuenta = cuenta.Cuenta(10000,150000,0,0.01,0)
